...
public class Sudoku {    
    ...
    /* 
     * isValid - check to see if it is valid to place val
     * in the specified row and col
     */
    public boolean isValid(int row, int col, int val) {
        
        return (!this.valIsFixed[row][col] && !this.subgridHasVal[row/3][col/3][val] && !this.rowHasVal[row][val] && !this.colHasVal[col][val]);
    }
     ...
     */
    private boolean solveRB(int n) {     // ADD IN YOUR PARAMETERS!
        if (n >= 81) {
            return true;
        }
        int row = n / 9;
